...
AXE_REPORTS_DIR = Path(os.getcwd()) / "axe-reports"
AXE_PATH = Path(os.getcwd()) / "src" / "pytest_playwright_axe" / "resources" / "axe.js"
MIN_AXE_PATH = Path(os.getcwd()) / "src" / "pytest_playwright_axe" / "resources" / "axe.min.js"
...
def test_initialize_axe() -> None:
    assert Axe(output_directory=Path(os.getcwd()) / "test-output-dir").output_directory == Path(os.getcwd()) / "test-output-dir"
    assert Axe(use_minified_file=True).axe_path == MIN_AXE_PATH
    default_axe = Axe()
    assert default_axe.output_directory == AXE_REPORTS_DIR
    assert default_axe.axe_path == AXE_PATH
...
def test_create_path_for_report() -> None:
    assert Axe()._create_path_for_report('test123.html') == Path(
